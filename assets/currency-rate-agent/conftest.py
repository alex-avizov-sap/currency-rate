...
@pytest.fixture(scope="session")
def add_agent_to_path(agent_path: Path, agent_app_path: Path):
    """Add app/ and the agent root to sys.path so agent modules can be imported directly.

    Both paths are needed because agent modules may use either style:
    - Peer imports like ``from agent import Foo`` (needs app/ on path)
    - Package imports like ``from app.tools import bar`` (needs agent root on path)
    """
    paths_to_add = [str(agent_app_path), str(agent_path)]
    added = []
